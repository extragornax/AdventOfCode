...
def codeOne(pa):
    par = twoParamDest(pa)
    pa['data'][par['destination']] = add(
        pa['data'][par['one']], pa['data'][par['two']])
    return pa['arrPos'] + 4


def codeTwo(pa):
    par = twoParamDest(pa)
    pa['data'][par['destination']] = mutl(
        pa['data'][par['one']], pa['data'][par['two']])
    return pa['arrPos'] + 4
...
def codeFive(pa):
    par = oneParamDest(pa)
    if int(pa['data'][par['one']]) != 0:
        # pa['data'][pa['arrPos']] = pa['data'][par['two']]
        return pa['data'][par['two']]
    return pa['arrPos'] + 3
...
def codeSeven(pa):
    par = twoParamDest(pa)
    if pa['data'][par['one']] < pa['data'][par['two']]:
        pa['data'][par['destination']] = 1
    else:
        pa['data'][par['destination']] = 0
    return pa['arrPos'] + 4
...
def main():
    ...
    while data[arrPos] != 99:
        ...
        # print("Info",data[arrPos], "Opcode:",opcode, "Pos:", arrPos, "Array:",data,"Params", params)
        if opcode == 1:  # add
            ...
            arrPos = codeFive(params)
        elif opcode == 6:  # jump-if-false
            arrPos = codeSix(params)
        elif opcode == 7:  # less than
            arrPos = codeSeven(params)
        elif opcode == 8:  # equals
            arrPos = codeEight(params)
        else:
            print("Error opcode", opcode, "doesn't exist")
            return
